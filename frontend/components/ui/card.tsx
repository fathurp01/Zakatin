...
function Card({
  ...
  return (
    <div
      ...
      className={cn(
        // Base
        "group/card flex flex-col gap-4 overflow-hidden text-sm/relaxed",
        // Shape & border
        "rounded-2xl border border-slate-200/60 dark:border-white/8",
        // Surface
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-sm text-slate-500 dark:text-muted-foreground leading-relaxed", className)}
      {...props}
    />
  )
}
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-lg px-5",
        "group-data-[size=sm]/card:px-4",
        "[.border-t]:pt-4 group-data-[size=sm]/card:[.border-t]:pt-3",
        className
      )}
      {...props}
    />
